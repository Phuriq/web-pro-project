  ...
  <div class="bg-black container fadeinleft animation-duration-200">
    ...
    <div class="mt-5 flex align-items-center">
      <div class="flex-grow-1 flex flex-column align-items-center px-5">
        <div
          class="flex justify-content-start align-items-center w-full row-gap-5"
          v-for="seatRow in seats"
          :key="seatRow.id"
        >
          <div class="mr-5 text-3xl">{{ seatRow.seatRow }}</div>
          <div class="flex justify-content-between column-gap-3">
            <div class="" v-for="seat in seatRow.items" :key="seat.id">
              <div
                @click="seat.reservations.length == 0 && handlerClickSeat(seat)"
                :class="[
                  this.chooseSeat.findIndex((item) => item.id === seat.id) !== -1 && 'text-red-400',
                  seat.seatNumber === 5 && 'ml-5',
                  seat.reservations.length != 0 && 'text-red-400'
                ]"
                class="text-3xl flex flex-column align-items-center"
              >
          ...
          <div class="ml-5 text-3xl">{{ seatRow.seatRow }}</div>
        </div>
      </div>
      <div class="flex col-4">
